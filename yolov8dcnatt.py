...
class MPCA(nn.Module):
    ...
    def forward(self, x):
        ...
        return x * x_pool_h.sigmoid() * x_pool_w.permute(0, 1, 3, 2).sigmoid() * x_pool_ch.sigmoid() + x
...
class YOLOv8s_Backbone(nn.Module):
    def __init__(self):
        ...
        self.layers = nn.Sequential(
            Conv(3, 48, 3, 2, 1),
            Conv(48, 96, 3, 2, 1),
            C2f_DCNv2_Dynamic(96, 96, 2, True),
            Conv(96, 192, 3, 2, 1),
            C2f_DCNv2_Dynamic(192, 192, 4, True),
            Conv(192, 384, 3, 2, 1),
            C2f_DCNv2_Dynamic(384, 384, 4, True),
            Conv(384, 576, 3, 2, 1),
            C2f_DCNv2_Dynamic(576, 576, 2, True),
            SPPF(576, 576)
        )
